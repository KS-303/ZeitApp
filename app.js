const DB_NAME='zeitapp_db_v7',DB_VERSION=1,STORE_ENTRIES='entries',STORE_SETTINGS='settings',STORE_META='meta';
...
async function render(){
 ...
 stats.innerHTML=`<div class="stat"><span>Soll</span><strong>${minutesToHours(soll)}</strong></div><div class="stat"><span>Ist</span><strong>${minutesToHours(totals.work)}</strong></div><div class="stat"><span>Urlaub/Krank/FT</span><strong>${minutesToHours(credit)}</strong></div><div class="stat"><span>Überstunden</span><strong>${overtime>=0?'+':''}${minutesToHours(Math.abs(overtime))}</strong></div><div class="stat"><span>Fahrzeit</span><strong>${minutesToHours(totals.drive)}</strong></div><div class="stat"><span>Pause</span><strong>${minutesToHours(totals.breaks)}</strong></div><div class="stat"><span>Kilometer</span><strong>${totals.km.toFixed(1)}</strong></div><div class="stat"><span>Feiertage gebucht</span><strong>${feiertage}</strong></div>`;
}
...
async function exportToGoogleSheets(){
 ...
 try{ const response=await fetch(webhook,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}); if(!response.ok) throw new Error('HTTP '+response.status); const result=await response.json().catch(()=>({ok:true}));
